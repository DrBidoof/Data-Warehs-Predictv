...
Train, tune, calibrate, evaluate, and save artifacts.
- Modes: dev | balanced | final (default: dev)
- Reads artifacts from exports/IO2/ and writes outputs to exports/IO3/
- Prints training/tuning results and final evaluation summary at the end
"""
import os
import time
import argparse
...
print(f"Mode: {MODE}; sample_frac={SAMPLE_FRAC}")
...
if SAMPLE_FRAC is not None and 0 < SAMPLE_FRAC < 1.0:
    print(f"Sampling {SAMPLE_FRAC*100:.1f}% of training data for heavy steps.")
...
print(f"LR search done in {time.time()-t1:.1f}s; best_score={gs_lr.best_score_:.6f}")
...
print(f"RF search done in {time.time()-t2:.1f}s; best_score={rs_rf.best_score_:.6f}")
...
print(f"DT search done in {time.time()-t3:.1f}s; best_score={gs_dt.best_score_:.6f}")
...
for name, model in models.items():
    ...
    print(f"{name} eval done in {time.time()-start:.1f}s; accuracy={acc:.4f}, roc_auc_ovr={auc}")
...
with open(os.path.join(OUT, "model_evaluation_summary.json"), "w", encoding="utf-8") as f:
    json.dump({"final_results": final_results,
               "cv_best_scores": {"lr": float(gs_lr.best_score_), "rf": float(rs_rf.best_score_), "dt": float(gs_dt.best_score_)},
...
# -------------------------
# Print training/tuning results summary (restored per request)
# -------------------------
print("\n=== TRAINING & TUNING SUMMARY ===")
try:
    print("\nLogisticRegression GridSearch best params:", gs_lr.best_params_)
    print("LogisticRegression best CV score (roc_auc_ovr):", float(gs_lr.best_score_))
except Exception:
    print("LogisticRegression results unavailable.")

try:
    print("\nRandomForest RandomizedSearch best params:", rs_rf.best_params_)
    print("RandomForest best CV score (roc_auc_ovr):", float(rs_rf.best_score_))
except Exception:
    print("RandomForest results unavailable.")

try:
    print("\nDecisionTree GridSearch best params:", gs_dt.best_params_)
    print("DecisionTree best CV score (roc_auc_ovr):", float(gs_dt.best_score_))
except Exception:
    print("DecisionTree results unavailable.")

print("\nSelected model:", best_name)
print("Final test accuracy:", final_results["accuracy"])
print("Final test ROC AUC OVR:", final_results["roc_auc_ovr"])
print("Confusion matrix (test):")
print(np.array(final_results["confusion_matrix"]))
print("\nClassification report (test):")
print(json.dumps(final_results["classification_report"], indent=2))

print("\nArtifacts saved to:", OUT)
print("03_train_evaluate_optimized.py complete.")
